...
#         self.attn_combine = nn.Linear(self.rnnHiddenSize * 2, self.rnnHiddenSize)
    ...
    def forward(self, encStates, inputSeq, enc_outputs = None, Questioner = False):
        '''
        ...
        if inputSeq is not None:
            if Questioner:
                    ...
                    self.attn = nn.Linear(self.rnnHiddenSize, self.rnnHiddenSize).cuda()
#                     self.concat = nn.Linear(self.rnnHiddenSize * 2, self.rnnHiddenSize).cuda()
                    self.concat = nn.Linear(self.rnnHiddenSize * 2, self.vocabSize).cuda()
...
#                     context = attn_weights.bmm(outputs.transpose(1,2))
#                     context = attn_weights.bmm(enc_outputs)
#                     print(context.shape)
                    output_context = torch.cat((outputs, context), 2)
                    # [ts, b, h] 线性层2h-h
                    flatScores = self.concat(output_context)
...
#                     logProbs = flatLogProbs.view(outputSize[0], outputSize[1], -1)
                    logProbs = self.my_log_softmax(flatScores)

                   
#                     output = torch.cat((embedded[0], attn_applied[0]), 1)
#                     output = self.attn_combine(output).unsqueeze(0)
#                     output = F.relu(output)
#                     outputs, _ = self.rnn(output, encStates)
